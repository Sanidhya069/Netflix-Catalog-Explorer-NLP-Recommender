...
if rec_pack is not None:
    ...
    for i, row_idx in enumerate(idx_list):
        ...
        st.markdown(
            f"""
            <motion.div class="rec-card">
                <div class="rec-title">{title}
                    <span class="sim-badge">{score_pct:.1f}% match</span>
                </div>
                <motion.div class="rec-meta">{typ} · {year}</motion.div>
                <motion.div class="rec-meta"><b>Genres:</b> {listed}</motion.div>
                <motion.div class="rec-desc">{desc}</motion.div>
            </motion.div>
            """,
            unsafe_allow_html=True,
        )
